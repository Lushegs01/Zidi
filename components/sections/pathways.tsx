        ...
        <div className="mt-12 grid gap-6 md:mt-14 lg:grid-cols-2 lg:gap-8">
          {paths.map((path, i) => (
            <Reveal key={path.href} delay={i * 90}>
              <Link
                href={path.href}
                className="group flex h-full flex-col overflow-hidden rounded-lg border border-line bg-bone shadow-soft transition-shadow duration-300 hover:shadow-lift"
              >
                {/* Image with eyebrow label overlaid on the photo */}
                <div className="relative aspect-16/10 overflow-hidden bg-sand">
                  <Image
                    ...
                    placeholder="blur"
                  />
                  {/* Warm kola overlay tint on hover */}
                  <div
                    aria-hidden="true"
                    className="absolute inset-0 bg-kola/0 transition-colors duration-500 group-hover:bg-kola/10"
                  />
                  {/* Eyebrow label sits ON the photo — editorial, anchored to the image */}
                  <p
                    aria-hidden="true"
                    className="absolute bottom-4 left-5 flex items-center gap-2 text-[0.6875rem] font-semibold uppercase tracking-[0.16em] text-bone"
                  >
                    <span className="h-1.5 w-1.5 rounded-full bg-amber" />
                    {path.eyebrow}
                  </p>
                  {/* Gradient scrim for photo legibility */}
                  <div
                    aria-hidden="true"
                    className="absolute inset-0 bg-linear-to-t from-kola/50 from-0% to-transparent to-50%"
                  />
                </div>

                <div className="flex flex-1 flex-col p-7 md:p-8">
                  {/* Visible eyebrow label in body — accessible, reads in sequence */}
