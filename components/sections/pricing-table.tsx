      ...
      >
        {plans.map((plan) => (
          <article
            key={plan.id}
            className={cn(
              "relative flex flex-col rounded-lg border p-6 transition-shadow duration-300 md:p-7",
              plan.recommended
                ? "border-clay bg-bone shadow-lift ring-1 ring-clay"
                : "border-line bg-bone shadow-soft hover:shadow-lift",
            )}
          >
            {plan.recommended && (
              /* Arch-tab badge — sits flush to the card top edge */
              <span
                className="absolute -top-[1.65rem] left-5 bg-clay px-4 pb-1 pt-1.5 text-[0.6875rem] font-semibold uppercase tracking-[0.12em] text-bone"
